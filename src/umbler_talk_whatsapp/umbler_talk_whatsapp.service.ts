...
//Types
import { UTalkNotesCreated, UTalkMessageCreated } from './umbler_talk_whatsapp.types'
...
export class UmblerTalkWhatsappService {
  ...
  async createContact(user: CreateUserLeadDto) {
    const createContact: { data: { contact: { id: string } } } = await this.httpService.post(`${process.env.UMBLER_API}/contacts`, {
      name: user.name,
  ...
  async AddNotes(notes : UTalkNotesCreated) {
    const createNotes = await this.httpService.post(`${process.env.UMBLER_API}/contacts/${notes.idContactUtalk}/notes`,
        {
          content: notes.note,
          organizationId: process.env.UBMLER_ORGANIZATION,
        },
  ...
  async SendMessage( message: UTalkMessageCreated) {
    ...
    const chat = await this.httpService.post(
